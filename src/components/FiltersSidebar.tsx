...
const categories = [
  { name: "Books", icon: Book, color: "from-[hsl(152,60%,42%)] to-[hsl(170,60%,38%)]", active: true },
  { name: "Electronics", icon: Monitor, color: "from-[hsl(38,92%,50%)] to-[hsl(28,92%,50%)]", active: false },
  { name: "Furniture", icon: Armchair, color: "from-[hsl(0,84%,60%)] to-[hsl(350,84%,55%)]", active: false },
];
        ...
        <div className="space-y-2.5 relative">
          {categories.map((cat) => (
            ...
            >
              ...
              <span className={`font-medium ${cat.active ? "text-foreground" : "text-muted-foreground"}`}>
                {cat.name}
              </span>
            </button>
          ))}
      ...
      <div className="bg-card rounded-2xl p-6 shadow-card border border-border/50">
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-xs text-muted-foreground bg-accent px-2 py-1 rounded-md">₹10K</span>
          </div>
        </div>
      </div>
